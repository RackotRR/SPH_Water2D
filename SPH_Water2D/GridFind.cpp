...
{ 
	...
	const double blockSize{ GetScaleK() * hsml };

	// grid size
	const size_t sizeX{ static_cast<size_t>(ceil((Params::x_maxgeom - Params::x_mingeom) / blockSize)) };
	const size_t sizeY{ static_cast<size_t>(ceil((Params::y_maxgeom - Params::y_mingeom) / blockSize)) };
	// create grid
	ParticlesGrid grid(sizeX, sizeY);
